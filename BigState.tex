...
\section{Improved Bigstate DMPF.}\label{sec:bigState}

Boyle et al.\cite{dmpf} introduced two new approaches to DMPF: an OKVS-based construction and the \emph{big-state} construction. 
...
\begin{figure}\small
    ...
    \textsf{bigstate-DPF}: On input $\share{A}\in [0,n)^t,\share{B}\in \mathbb{G}^t$, party $p\in \{0,1\}$ computes:
    \begin{enumerate}
    ...
    \end{enumerate}
    \vspace{10px}
    \SharedIPD: On input $\share{u}\in \{x\in\{0,1\}^m \mid |x|\leq 1\}$ and $\share{v}\in \{x\in\{0,1\}^m \mid |x|\leq t\}$, the parties $p\in \{0,1\}$ compute:
    \begin{enumerate}
    ...
    \end{minipage}}
\caption{%
  Big-state Distributed Multi-Point Function (DPF) protocol \cite{dmpf}. Does not include our optimized \SharedIPD protocol.  \label{fig:BS-Dmpf}
}
\end{figure}
...
To obtain these pairs, we want to compute the inner product of the unit vectors $\share{\tau^{d-1}_i}\in \{0,1\}^{2^{d-1}}$ with the weight (at most) $t$ vector $\share{s'}$, where we view each pair of siblings in $s'$ as a single element. \cite{dmpf} gave a protocol for doing this called  $\SharedIPD$. Unfortunently, this routines is inefficient and only better than a trivial linear time solution when $n \gg 2^{30}$. Their approach is reproduced in \SharedIPD of \figureref{fig:BS-Dmpf}. The idea is to perform the inner product not on the seed+tag vector with $\{0,1\}^{2\kappa+2t}$ elements but one bit at a time. That is, compute the inner product between a unit vector $\share{\tau^{d-1}_i}\in \{0,1\}^{2^{d-1}}$ and a vector $\share{v}$ with weight at most $t$. Given this, one can compute the general case by using $2\kappa+2t$ calls to  $\SharedIPD$, one for each bit of the seed and tag vectors.
...
We improve the basic idea of \cite{dmpf} by giving a more efficient protocol that also directly work when the weight $t$ vector has strings for elements, as opposed to bits. That is, $u\in\{0,1\}^m$ is a \emph{unit} vector; $v\in \{0,1\}^{\sigma \times m}$ has Hamming weight $\le t$ when viewed as vector of $\{0,1\}^\sigma$ elements. Given secret shares $\share{u},\share{v}$, we want $\share{\langle u,v\rangle}$ with only:
